 ...
 */ 
Clarifai.prototype._commonHttpStatusHandler = function(  res, responseData, localId, resultHandler, retry, expR ) {
  console.log('COMMON IS: ', expR.toString());
...
Clarifai.prototype._httpRequest = function( endpoint, form, localId, resultHandler, retry, expR  )
{
  console.log('HTTP:', expR.toString());
  var responseData = '';
  ...
  var req = https.request( {
    ...
    res.on("data",function(chunk) { responseData += chunk; } );
    res.on("end",function() { 
      console.log('HTTP END', expR.toString());
      self._commonHttpStatusHandler( res, responseData, localId, resultHandler, retry, expR );
      });
...
Clarifai.prototype._tagURL  = function( url, localId, resultHandler, retry, expR ) {
  console.log(url);
  console.log(localId);
  ...
  this._httpRequest( tagPath, form, localId, resultHandler, retry, expR );

}

Clarifai.prototype.tagURL = function( url, localId, callback, expR ) {

  retry = function() {
      this.tagURL( url, localId, callback, expR );
  }.bind(this);
